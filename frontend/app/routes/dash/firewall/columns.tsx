...
const actionStyles: Record<FirewallRule["action"], string> = {
  ...
  Challenge:
    "bg-blue-100 text-blue-700 border border-blue-200 dark:bg-blue-900/30 dark:text-blue-400 dark:border-blue-700",
}
...
export const columns: ColumnDef<FirewallRule>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return (
        <span
          className={`rounded px-2 py-1 text-xs font-medium ${actionStyles[action]}`}
        >
          {action}
        </span>
      )
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center gap-2">
          <span
            className={`inline-flex h-2 w-2 rounded-full ${enabled ? "bg-green-500" : "bg-zinc-400 dark:bg-zinc-600"}`}
          />
