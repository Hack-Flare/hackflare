...
fn extract_ns_and_glue(
    ...
    for (rtype, rpos, _rdlen, _ttl) in authority_rrs {
        if *rtype == 2 {
            if let Some((name, _)) = parse_qname(buf, *rpos) {
                ns_names.push(name);
            }
        }
    }
    for (rtype, rpos, rdlen, _ttl) in additional_rrs {
        if *rtype == 1 && *rdlen == 4 {
            ...
            if let Ok(ipv6) = <[u8; 16]>::try_from(&buf[*rpos..*rpos + 16]) {
...
pub fn resolve(name: &str, qtype: u16, max_depth: usize) -> Option<Vec<u8>> {
    ...
    static CACHE: Lazy<Mutex<HashMap<(String, u16), (Vec<u8>, Instant)>>> =
            ...
            if let Some(resp) = send_recv(&sock, srv, &req) {
                ...
                if ancount > 0 {
                    if let Some(ans_rrs) = parse_rrs(&resp, pos, ancount) {
                        ...
                        for (rtype, rpos, rdlen, ttl) in &ans_rrs {
                            if *rtype == qtype {
                                if let Ok(mut c) = CACHE.lock() {
